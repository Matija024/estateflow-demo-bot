  ...
  return <div className={cn("bg-estate-bg-secondary border-r border-estate-border flex flex-col h-full transition-all duration-300", isExpanded ? "w-80" : "w-16")}>
      <div className="p-4 border-b border-estate-border">
        <div className="flex items-center justify-between mb-4">
          {isExpanded && <h2 className="text-lg font-semibold text-estate-text-primary">Wissen</h2>}
          <Button variant="ghost" size="sm" onClick={() => setIsExpanded(!isExpanded)} className="text-estate-text-secondary hover:text-estate-text-primary ml-auto hover:bg-estate-bg-primary">
            {isExpanded ? <ChevronLeft size={16} /> : <ChevronRight size={16} />}
          </Button>
        </div>
        
        {isExpanded && <div className="relative">
            <input type="file" accept=".pdf,.doc,.docx" multiple onChange={handleFileUpload} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
      ...
      {isExpanded && <div className="flex-1 overflow-y-auto p-4">
          {documents.length === 0 ? <div className="text-center py-8">
              ...
              {documents.map(doc => <Card key={doc.id} className="p-3 border-estate-border hover:shadow-card transition-smooth">
                  <div className="flex items-start gap-3">
                    <Checkbox checked={doc.selected} onCheckedChange={checked => onDocumentSelect(doc.id, checked as boolean)} className="mt-0.5 border-estate-border data-[state=checked]:bg-estate-purple data-[state=checked]:border-estate-purple" />
